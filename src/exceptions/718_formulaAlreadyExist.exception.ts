...
class FormulaAlreadyExistException implements HttpException {
  status : number;
  message: string;

  constructor(inputMessage?: string) {
    this.status  = 718;
    this.message = inputMessage ? inputMessage : "Formula already exist";
  }
...
export default FormulaAlreadyExistException;
