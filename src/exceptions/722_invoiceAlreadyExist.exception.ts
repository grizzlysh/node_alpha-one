...
class InvoiceAlreadyExistException implements HttpException {
  status : number;
  message: string;

  constructor(inputMessage?: string) {
    this.status  = 722;
    this.message = inputMessage ? inputMessage : "Invoice already exist";
  }
...
export default InvoiceAlreadyExistException;
