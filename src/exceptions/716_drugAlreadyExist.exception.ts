...
class DrugAlreadyExistException implements HttpException {
  status : number;
  message: string;

  constructor(inputMessage?: string) {
    this.status  = 716;
    this.message = inputMessage ? inputMessage : "Drug already exist";
  }
...
export default DrugAlreadyExistException;
